...
> **版本**：v1.8.1 · 2026-07-19  
> **Base URL**：`https://veslune.aabbaq.com/api/v1`  
> **范围**：iOS 前端对接 Backend REST API  

变更摘要（v1.8.1）：Profile `body` 新增外观字段 `gender_en` / `skin_tone_en` / `age_range_en` / `hair_style_en` / `hair_color_en`（及对应中文展示字段）；`profile_completed` 仍只看身高+体重。`POST /users/me/model` 对空字段从 Profile 回填；合并后仍缺 `gender` 或 `body_type` 返回 `40001`。
...
未填的外观字段为 `null`。`completed` / `profile_completed` **不要求**填写 `gender_en`。
...
| `gender_en` | string \| null | 否 | 性别，见 `type=gender`；**不强制** Profile 完成，但生成模特时合并后必填 |
| `skin_tone_en` | string \| null | 否 | 肤色，见 `type=skin_tone` |
| `age_range_en` | string \| null | 否 | 年龄段，见 `type=age_range` |
| `hair_style_en` | string \| null | 否 | 发型，见 `type=hair_style` |
| `hair_color_en` | string \| null | 否 | 发色，见 `type=hair_color` |
...
**Profile 回填**：请求里为空的字段，服务端用 `GET /users/me/profile` 的对应值补全：

| 请求字段 | Profile 来源 |
|----------|--------------|
| `gender` | `body.gender_en` |
| `body_type` | `body.body_type_en` |
| `height_cm` | `body.height_cm`（请求为 `0`/省略时回填） |
| `skin_tone` | `body.skin_tone_en` |
| `age_range` | `body.age_range_en` |
| `hair_style` | `body.hair_style_en` |
| `hair_color` | `body.hair_color_en` |

请求非空值优先。合并后若仍缺 `gender` 或 `body_type`（或枚举非法）→ `40001`。Profile 不强制填性别，但**生成模特时**必须能从请求或 Profile 得到性别。
...
若 Profile 已保存外观字段，可只传 overrides，例如：
...
| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `gender` | string | 合并后必填 | 见 `GET /metadata/enums?type=gender`；可省略若 Profile 已有 `gender_en` |
| `body_type` | string | 合并后必填 | 见 `type=body_shape`（与 Profile `body_type_en` 共用） |
| `height_cm` | float | 否 | 身高 cm；`0`/省略时用 Profile |
| `skin_tone` | string | 否 | 见 `type=skin_tone` |
| `age_range` | string | 否 | 见 `type=age_range` |
| `hair_style` | string | 否 | 见 `type=hair_style` |
| `hair_color` | string | 否 | 见 `type=hair_color` |
