...
## 四层阶梯（先记住这张图，全程按它对账）
```
① 需求（SRS）      要什么 & 为什么          —— docs/SRS-3D-tryon.md
        ↓ 只与相邻层对账
...
## 前置阅读（新对话里先读）
- `docs/SRS-3D-tryon.md` —— 重点 **§4 技术架构**、**§4.5 AIKit 边界**、**§6.4 两类 Provider**、**§8 端上直连与密钥管理**、**§7.2 数据模型**、约束 **C-5/C-6**、**NFR-2/3/6**、附录 A 追踪矩阵
...
- docs/SRS-3D-tryon.md（重点 §4 架构 / §4.5 AIKit 边界 / §6.4 两类 Provider / §8 端上直连与密钥管理 / §7.2 / C-5 / C-6 / NFR-2/3/6 / 附录A）
