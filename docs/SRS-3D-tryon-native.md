# 软件需求规格说明书（SRS）— 3D 虚拟试衣 App（**第二阶段 · 原生 Swift + RealityKit**）

> 🧭 **阶段定位（2026-07 技术路线「Web 先行」拍板后）**：本文件描述的是 **第二阶段·原生移植** 的 SRS（Swift + RealityKit + USDZ + SwiftData + iOS 18）。
> **第一阶段·本期主线是 Web（TS + WebGL2 + glTF/GLB + IndexedDB）**，另立一份 SRS（见 `backlog.md` §A「Web 先行」🔒）。
> 本文件的**产品范围/功能需求/资产管线/AI 纹理管线/数据模型概念**大多与栈无关、可为第一阶段复用；**技术架构/资产格式/存储/最低系统**为原生栈专属。

> 产品（暂定名）：**3D 试衣分身 / FitTwin 3D**（暂定，待定名）
> 文档版本：v0.1（草案，待评审）
> 编写日期：2026-06-29　｜　阶段：**第二阶段·原生**
> 适用平台：iOS（仅 iOS），**最低 iOS 18**（原生阶段门槛；Web 阶段不受此限）
> 状态：MVP 需求基线
...
### 10.1 各里程碑退出标准（Exit Criteria）
> 满足才算"完成"；作为评审/验收清单。

**M0 资产管线打通**
- [ ] ≥1 基础人体 + 3~5 件 conforming 衣服，**分开法**独立 usdz。
- [ ] 身体与每件衣服带 §5.2 **同名通道子集**；prim 名 / `mat_basecolor` 槽 / 单一 UV `st` 符合 §5.4。
- [ ] **资产 A–E 复验通过**（§5.7）。
- [ ] 每件资产带 **manifest**（§5.9）并登记进 **AssetIndex**（§5.5）。

**M1 捏人 MVP**
- [ ] 加载资产（正式或占位），折叠面板驱动 8~10 通道 + 骨骼缩放，实时形变 **≥60fps**（NFR-1）。
- [ ] **锁视角 → 正交测量态**：正面进入、箭头循环命名视图、标尺 + 实时尺寸读数（B-2 标定表接好）；**RISK-10 spike 已有结论**。
- [ ] FR-6 透视自由旋转 ⇄ 正交测量切换带过渡动画。
- [ ] 模特状态可存/取（§7.2 `BodyModel`）。
...
- [ ] ARKit 自动量身在支持机型 seed 比例通道、不支持回退手动（FR-2 / NFR-4）。
- [ ] 搭配存档/复用（`Outfit` / §7.2）。

**M4 合规与收尾**
- [ ] 同意/披露流 + 数据分级留存 + 一键删除（NFR-3）。
- [ ] 上传/生成图内容审核（NFR-7）。
- [ ] 包体 **≤~200MB**（NFR-8）；性能/崩溃率达标、可提审。
