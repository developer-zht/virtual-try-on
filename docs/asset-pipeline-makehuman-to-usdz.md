...
## 正式资产路线图（Phase 2，先记下、现在不做）

> 本节只记录**终态设计**，spike 阶段**仍只做 1 个通道**（见上文）。终点不变，证伪逻辑不破坏。

### 终态：参数化人体 = 一个网格 + 一组 blendshape 通道
正式 `scene.usdz` 的 Body / 衣服应各带同一套通道（运行时只改权重、永不重导模型）：
`k_weight` · `k_muscle` · `k_height` · `k_shoulder` · `k_hip` · `k_chest` · `k_waist` · `k_legLength` · `k_armLength`（`k_faceWidth` 可选）。

### Phase 2 做法 = Phase 1 的机械重复
对每个滑块：在 MakeHuman 里**先把所有滑块归零、只拉这一个到 Max → 导出**，到 Blender 对同一个 neutral Body 做 `Join as Shapes`，命名 `k_xxx`。Body 最终 = `Basis + k_weight + k_muscle + …`；衣服同理、**通道名一一对应**。
- ⚠ **纪律**：每个 Max 导出必须**只动那一个滑块**，否则该通道差值会被别的滑块污染。
- ⚠ **Phase 2 专属新风险（Phase 1 看不到）**：多个 blendshape 移动**重叠顶点**时，线性叠加会相互污染/过度形变（weight+waist+hip 同时拉满可能把躯干吹爆）。这是独立的质量问题，**留到 Phase 2 单独验**，不要和「管线通不通」混在一起调。

### 工具选择与 spike 解耦（重要）
spike 验证的是 **Blender→USD→RealityKit** 这半条链（blendshape 能否存活并被驱动），**与用什么工具捏人体无关**——MakeHuman / MB-Lab / Human Generator / CC4 产出的身体，都走同一条 USD 路。所以：
- **spike 现在用 MakeHuman 仅作"一次性测试资产来源"**（免费、即取即用），**不等于把它定为正式生产工具**。
- 正式生产工具（含是否换 CC4/Human Generator 等）是**另一个决策**，可在 spike 通过后再做。届时核心约束是**许可**：本 App「让用户拖滑块改身材」属 character-creation 用法 → **CC4 大概率触发 Enterprise License**；**MakeHuman 导出 CC0 无条件**最干净。详见 `feasibility-3D-tryon.md` §1/§2。

## 概念补充（两个最容易吓到人的点）

### 坐标轴朝向：Y-up vs Z-up
- 各软件对「哪个轴朝上」约定不同：**Y-up**（WebGL/OpenGL/three.js、USD、RealityKit、Unity、Maya）vs **Z-up**（Blender、3ds Max、CAD）。
- 所以 Blender 里人沿 **Z** 站，RealityKit 期望沿 **Y** 站。导出 USD 时勾 **`Convert Orientation`**（默认 Forward=-Z, Up=Y）= 把 Z-up 转成 Y-up。
- 需要 **Up（哪边朝上）+ Forward（脸朝哪）两个轴**才能把姿态定死（光有"上"还能绕竖直轴乱转）。默认值即标准换算，不用改。
- Blender 5.x 没有独立的「Up Axis」选项，它被收进 `Convert Orientation` 开关里（勾上才展开 Forward/Up 下拉）。
- 注意：此项即使设错，最坏是模型在 App 里躺倒，**代码一行可转正，不影响 blendshape 验证**。

### USD 的 blendshape 必带一根「Skel」骨架
- 导出后重新导入，会发现 Body 下多了个 `Skel`（骨架）、视图里有个八面锥 gizmo——**这是正常的、预期的**。
- 原因：**USD 规范规定 blendshape 必须挂在 UsdSkel 骨架上**，所以 Blender 自动塞了一根占位骨架来承载 shape key。
- 那个锥只是**编辑器里的骨骼显示图标，不是可渲染几何体**；它和身体的「穿模」纯属图标视觉重叠，**RealityKit 里不会渲染骨骼**。
- 反而是好事：说明 USD 是标准「带 blendshape 的蒙皮网格」，正是 `BlendShapeWeightsComponent` 期望的格式。
