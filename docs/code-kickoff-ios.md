...
> ⚠️ **阶段定位**：本文属 **第二阶段·原生（RealityKit）**。2026-07 已拍板 **Web 先行**——**第一阶段请先做 Web（TS+WebGL2）**，本 iOS spike/脚手架留到原生移植阶段再用（Web 端正交相机是作者主场、几乎无风险，RISK-10 那类不确定性在 Web 阶段不存在）。
> **用途**：写代码是大活，建议**开新对话**（干净上下文）。把本文末尾「开场白」代码块整段复制到新对话即可启动 **任务①（RISK-10 正交相机/投影过渡 spike）** 与 **任务②（iOS 脚手架）**。
> 本文与 SRS 配套：SRS 在分支 `claude/3d-tryon-srs-svsasl` 的 `docs/SRS-3D-tryon-native.md`。
> 这两个任务**不依赖 M0 正式资产**——用 spike 已产出的 `scene.usdz` 当占位即可起步。

---

## 前置：先读这些（新对话里）
- `docs/SRS-3D-tryon-native.md` —— 本产品 SRS（重点 §4 技术架构、§5.2/§5.4 契约、§7.2 SwiftData、FR-1/FR-6 锁视角测量、RISK-10）
...
- docs/SRS-3D-tryon-native.md（重点 §4 架构、§5.2/§5.4 契约、§7.2 SwiftData、FR-1/FR-6、RISK-10）
